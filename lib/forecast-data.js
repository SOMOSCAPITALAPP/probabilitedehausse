...
function horizonScenarioDrift(asset, horizonKey, annualDividendYield = 0, beta = 1) {
  ...
  const signalTilt = normalizedSignal * historicalVol * 0.35;
  const carry = annualDividendYield * (days / 252);
  const betaPenalty = beta > 1.15 ? -0.0015 * (days / 21) : 0;

  return historicalMean + signalTilt + carry + betaPenalty;
}
...
export function buildBetaScenarioAnalysis(asset, yahooSnapshot) {
  ...
  const buildScenarioRow = (label, marketMove, horizonKey) => {
    ...
    const sensitivity = beta * marketMove;
    const probableReturn = clamp(drift + sensitivity, -0.95, 1.5);
