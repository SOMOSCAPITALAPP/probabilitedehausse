...
export function buildAssetPathStudy(asset, historyRows) {
  ...
  const templateMeanDailyReturn = average(lastYearReturns) || 0;
  const centeredTemplateReturns = lastYearReturns.map((value) => value - templateMeanDailyReturn);
  const centeredTemplatePath = centeredTemplateReturns.reduce((sum, value) => sum + Math.abs(value), 0);
  const volatilityScale = targetPathLength / Math.max(0.0001, centeredTemplatePath || lastYearPath);
  const driftPerDay = projectedTerminal / 252;

  let projectedDailyReturns = centeredTemplateReturns.map((value, index) => {
    const day = index + 1;
    const bias = interpolateAnchors(biasAnchors, day).cumulative;
    const amplitude = Math.abs(value) * volatilityScale;
    const volatilityPulse = amplitude * bias * 0.7;
    return driftPerDay + value * volatilityScale + volatilityPulse;
  });

  let adjustedPathLength = projectedDailyReturns.reduce((sum, value) => sum + Math.abs(value), 0);
  if (adjustedPathLength < targetPathLength) {
    const extraScale = targetPathLength / Math.max(0.0001, adjustedPathLength);
    projectedDailyReturns = projectedDailyReturns.map((value, index) => {
      const day = index + 1;
      const bias = interpolateAnchors(biasAnchors, day).cumulative;
      const excess = value - driftPerDay;
      return driftPerDay + excess * extraScale * (1 + Math.abs(bias) * 0.15);
    });
  }

  let projectedLevel = 100;
  let projectedSeries = [{ day: 0, value: 0 }];
  for (let index = 0; index < projectedDailyReturns.length; index += 1) {
    projectedLevel *= 1 + projectedDailyReturns[index];
    projectedSeries.push({
      day: index + 1,
      value: projectedLevel / 100 - 1,
    });
  }

  const compoundedTerminal = projectedSeries[projectedSeries.length - 1]?.value ?? 0;
  const terminalAdjustment = projectedTerminal - compoundedTerminal;
  if (Math.abs(terminalAdjustment) > 0.0001) {
    projectedSeries = projectedSeries.map((point) => ({
      day: point.day,
      value: point.value + terminalAdjustment * (point.day / 252),
    }));
  }
