...
export const DASHBOARD_HORIZON_ORDER = ["5D", "21D", "63D"];
...
export function outcomeLabel(forecast) {
  ...
  if (expectedReturn > 0 && probability >= 0.6 && expectedDrawdown <= 0.04) {
    return "favorable";
  }
  if (expectedReturn > 0 && probability >= 0.5) {
    return "constructive";
  }
  if (expectedReturn <= 0 || probability < 0.45) {
    return "defensive";
  }
  return "mixed";
}
...
export function groupForecastsByAsset(forecasts) {
  ...
  assets.sort((left, right) => {
    const rightScore = (right.average_probability ?? 0) + right.positive_horizons * 0.05 - right.defensive_horizons * 0.05;
    const leftScore = (left.average_probability ?? 0) + left.positive_horizons * 0.05 - left.defensive_horizons * 0.05;
