...
async function fetchYahooAssetSnapshotFromSymbol(symbol) {
  ...
  if (!response.ok) {
    throw new Error(`Yahoo quoteSummary failed for ${symbol}: ${response.status}`);
  }
  ...
  return {
    marketCap: firstDefined(result?.price?.marketCap?.raw, result?.defaultKeyStatistics?.marketCap?.raw),
    beta: firstDefined(result?.defaultKeyStatistics?.beta?.raw, result?.summaryDetail?.beta?.raw),
    peRatio: firstDefined(result?.summaryDetail?.trailingPE?.raw, result?.defaultKeyStatistics?.trailingPE?.raw),
    eps: firstDefined(result?.defaultKeyStatistics?.trailingEps?.raw),
    earningsDate: firstDefined(firstEarningsDate?.fmt, firstEarningsDate?.raw),
    forwardDividend: firstDefined(result?.summaryDetail?.dividendRate?.raw),
    dividendYield: firstDefined(result?.summaryDetail?.dividendYield?.raw),
    exDividendDate: firstDefined(result?.summaryDetail?.exDividendDate?.fmt, result?.summaryDetail?.exDividendDate?.raw),
    targetPrice: firstDefined(result?.financialData?.targetMeanPrice?.raw),
    currency: firstDefined(result?.price?.currency),
  };
}
