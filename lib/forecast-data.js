...
export function buildAssetPathStudy(asset, historyRows) {
  ...
  const averageVol = assetAverageVolatility(asset);
  const currentVol = assetCurrentVolatility(asset);
  const projected = buildProjectedPathSeries(asset);
  ...
  const targetPathLength =
    ...
    clamp(
      0.9 +
        (currentVol / Math.max(0.0001, averageVol || currentVol || 0.0001)) * 0.5 +
        Math.abs(projectedTerminal - meanAnnualReturn) * 1.6,
      0.9,
      2.8
    );
  ...
  const waveSeries = projected.points.map((point) => point.cumulative);
  let projectedSeries = projected.points.map((point) => ({ day: point.day, value: point.cumulative }));
  const currentPathLength = projectedSeries.reduce((sum, point, index) => {
    if (index === 0) return 0;
    return sum + Math.abs(point.value - projectedSeries[index - 1].value);
  }, 0);

  if (currentPathLength > 0) {
    const scale = targetPathLength / currentPathLength;
    projectedSeries = projectedSeries.map((point, index) => {
      const base = baseLine[index].value;
      return {
        day: point.day,
        value: base + (waveSeries[index] - base) * scale,
      };
    });
  }
  projectedSeries = fitSeriesInsideBeam(projectedSeries, baseLine, targetPathLength);

  const trailingSeries = [{ day: 0, value: 0 }];
  let cumulative = 0;
  for (let index = 0; index < lastYearReturns.length; index += 1) {
    cumulative += lastYearReturns[index];
    trailingSeries.push({ day: index + 1, value: cumulative });
  }
  const fittedTrailingSeries = fitSeriesInsideBeam(trailingSeries, baseLine);
  ...
  return {
    ...
    lowerLine,
    projectedSeries,
    trailingSeries: fittedTrailingSeries,
    averageVol,
    currentVol,
  };
}
