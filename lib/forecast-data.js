...
export function buildAssetPathStudy(asset, historyRows) {
  ...
  function capSeriesInsideBeam(series, referenceLine, tolerance = 0.98) {
    return series.map((point, index) => {
      const base = referenceLine[index]?.value ?? 0;
      const upper = upperLine[index]?.value ?? base;
      const lower = lowerLine[index]?.value ?? base;
      const maxUp = Math.max(0, (upper - base) * tolerance);
      const maxDown = Math.max(0, (base - lower) * tolerance);
      const deviation = point.value - base;

      return {
        day: point.day,
        value: base + clamp(deviation, -maxDown, maxUp),
      };
    });
  }
  ...
  const cappedProjectedIndexedSeries = projectedIndexedSeries.map((point, index) => {
    ...
    return {
      day: point.day,
      value: clamp(point.value, allowedLow, allowedHigh),
    };
  });
