...
function getAssetBenchmarkConfig(asset) {
  if (!asset?.source_symbol) {
    return null;
  }
...
export async function getReferenceBenchmarkHistory(asset) {
  const benchmark = getAssetBenchmarkConfig(asset);
  if (!benchmark) {
    return [];
  }

  return getAssetHistory(benchmark.benchmarkCode);
}

export function buildBenchmarkRelativeStudy(asset, assetHistoryRows, benchmarkHistoryRows) {
  const benchmark = getAssetBenchmarkConfig(asset);
  if (!asset || !benchmark) {
    return null;
  }
