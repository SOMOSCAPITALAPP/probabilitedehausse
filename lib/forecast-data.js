...
export function buildAssetPathStudy(asset, historyRows) {
  ...
  let trailingLevel = 100;
  const trailingCumulativeReturns = [0];
  for (let index = 0; index < lastYearReturns.length; index += 1) {
    trailingLevel *= 1 + lastYearReturns[index];
    trailingCumulativeReturns.push(trailingLevel / 100 - 1);
  }

  const pathRatio = clamp(targetPathLength / Math.max(0.0001, lastYearPath), 0.95, 1.35);
  const volatilityRatio = clamp(currentVol / Math.max(0.0001, averageVol || currentVol || 0.0001), 0.95, 1.2);
  const terminalShift = projectedTerminal - meanAnnualReturn;

  const projectedSeries = trailingCumulativeReturns.map((cumulativeReturn, index) => {
    const day = index;
    const lastYearLine = lastYearReturn * (day / 252);
    const historicalDeviation = cumulativeReturn - lastYearLine;
    const bias = interpolateAnchors(biasAnchors, day).cumulative;
    const envelope = Math.sin((Math.PI * day) / 252);
    const deviation = historicalDeviation * pathRatio * volatilityRatio;
    const biasAdjustment = envelope * Math.abs(historicalDeviation) * bias * 0.9;

    return {
      day,
      value: baseLine[index].value + terminalShift * (day / 252) + deviation + biasAdjustment,
    };
  });

  const cappedProjectedSeries = capSeriesInsideBeam(projectedSeries, baseLine, 0.985);
  ...
  return {
    ...
    projectedSeries: toIndexedSeries(cappedProjectedSeries),
    trailingSeries,
    averageVol,
    currentVol,
  };
